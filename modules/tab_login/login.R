  ...
  shiny::fluidRow(
    shiny::column(
      ...
      bs4Dash::box(
        width = NULL,
        status = "primary",
        title = i18n$t("login", session = NULL),
        solidHeader = TRUE,
        shiny::uiOutput(
          outputId = ns("login")
        )
    ...
    function(input, output, session) {
      ...
      login_r <- shiny::reactive({
        name_input <- if (.values$yaml$showcase) {
          shiny::selectInput(
            ...
            label = i18n$t("user_name", session = NULL),
        ...
        htmltools::tagList(
          ...
          shiny::passwordInput(
            ...
            label = i18n$t("password", session = NULL),
            placeholder = "1234"
          ),
          shiny::actionButton(
            inputId = ns("user_login"),
            label = i18n$t("login", session = NULL),
            width = "100%"
          )
        )
      })
      ...
      shiny::observeEvent(input$user_login, {
        ...
        if (pwd_correct) {
          ...
          shiny::showNotification(
            ui = i18n$t("msg_login_successful", session = NULL),
            type = "default",
            duration = 3
          )
        } else {
          shiny::showNotification(
            ui = i18n$t("msg_wrong_password", session = NULL),
            type = "error",
            duration = 3
          )
        }
      ...
      logout_r <- shiny::reactive({
        shiny::actionButton(
          ...
          label = i18n$t("logout", session = NULL),
          width = "100%"
        )
      })
      ...
      shiny::observeEvent(input$user_logout, {
        ...
        shiny::showNotification(
          ui = i18n$t("msg_logout_successful", session = NULL),
          type = "default",
          duration = 3
        )
      })
      ...
      output$password_tbl_container <- shiny::renderUI({
        if (.values$yaml$showcase) {
          bs4Dash::box(
            ...
            status = "primary",
            solidHeader = TRUE,
            title = i18n$t("passwords", session = NULL),
            htmltools::p(
              i18n$t("table_only_visible_test_version", session = NULL)
            ),
